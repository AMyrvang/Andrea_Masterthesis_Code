...
# Local dependencies
from IEnKS import tsubspaceEnKA
...
def get_synthetic_truth_data() -> pd.DataFrame:
    ...
    perturbed_st_data_path: Path = Path('/cluster/projects/nn9774k/palm/v24.04/JOBS/Processed_files_flight_21')/FNAME #Path(PROJECT_ROOT / "Data" / "Processed_LES_data" / FNAME)

    #FNAME = "flight_21_1_Processed.csv"
    #perturbed_st_data_path: Path = Path("../Data/Processed_Drone_data")/FNAME
    return pd.read_csv(perturbed_st_data_path, index_col=False) #[[var for var in variables_to_assimilate.keys()]]


def create_perturbed_obs_matrix(obs_df, number_of_ensemble_members):
    #avg_number_of_aggregated_obs = 5 
    avg_number_of_aggregated_obs = obs_df['timestep_counter'].mean() #avg_number_of_aggregated_obs = 5  
    number_of_kalman_iterations = 1 # This is for the non iterative EnKS, fix later if iterations. 
...
def get_les_data() -> List[pd.DataFrame]:
    ...
    #les_data_paths = [f'/cluster/projects/nn9774k/palm/v24.04/JOBS/Processed_files_flight_21/zac_shf_{i}_output.csv' for i in itertools.chain(range(1,(64+1)))]

    for i, les_path in enumerate(les_data_paths):
        print(les_path)
        current_df = pd.read_csv(les_path, index_col = False)
        theta_values = current_df['theta']
        #u_values = current_df['u']
        #v_values = current_df['v']
...
def get_les_params() -> pd.DataFrame:
    ...
    #FNAME = "priors_shfl_shfs_ug_vg_pt_128n.csv"
...
def align_data(
    ...
    model_datetime_col: str,
    target_datetime_col: str,
):

    # Ensure datetime columns have datetime data type
...
def main() -> None:
    ...
    updated_parameter_matrix.to_csv('../Data/Syntetic_run_DA/syntetic_updated_parameter_matrix_with_15_as_truth.csv')
